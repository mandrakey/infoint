...
[code,sql]
----
SELECT n.name, n.country, count(q.qgram) AS nr FROM qgrams AS q JOIN names AS n ON n.id =q.name_id WHERE q.qgram IN ('Hu', 'ub', 'bi', 'ie') GROUP BY name HAVING nr > 2 ORDER BY nr DESC;
----
